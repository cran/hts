...
{ 
    ...
    fulldata <- full.hts(data)
    insamp.porps <- matrix(NA,nr=nrow(fulldata$gma), nc=ncol(fulldata$gma))
    for(i in 1: nrow(insamp.porps))
    {
        ...
        if(length(data$m)>1)
            ...
            {
                insamp.porps[i,((index[j-1]+1):(index[j]))] <- split(fulldata$gma[i,],rep(1:length(data$m),data$m))[[j]]/
                                                            split(fulldata$gma[i,],rep(1:length(data$m),data$m))[[j]]%*%scoef.l[[j-1]]
            }
    }
    ...
    tdmA.l <- ts(tdmA.l[[length(data$m)]],start=end(data$y)[1]+1,f=frequency(data$y))
